    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function renderPhaseDetails(phaseId) {
            ...
            const createAccordionItems = (items) => {
                ...
                return items.map(item => {
                    ...
                    return `
                    <details class="info-accordion" ${roleAttribute}>
                        <summary class="text-gray-800 hover:text-teal-700">${item.summary}</summary>
            ...
            const createSectionHtml = (section) => {
                ...
                if (section.type === 'output') {
                    ...
                    itemsHtml = `<div class="space-y-2">${createAccordionItems(section.items)}</div>`;
                }
                return `<div class="mb-4"><h4 class="sub-section-title">${sectionTitle}</h4>${itemsHtml}</div>`;
            };

            let sectionsHtml = phaseContent.sections.map(createSectionHtml).join('');
            let gateHtml = '';
        ...
        function renderTabContent(tabId) {
            ...
            if (tabId === 'chart' && typeof Chart !== 'undefined') {
                renderEffortChart();
            }
           if(tabId !== 'roles') { 
                applyRoleFilter(currentRole); 
